...
void monoalphabetic::set_plaintext(const std::string &p)
{
    try
    {
        if (p.empty())
        {
            throw std::invalid_argument("\033[1;31mPlaintext cannot be empty.\033[0m");
        }
        plaintext = p;
    }
    catch (const std::invalid_argument &e)
    {
        std::cerr << "\033[1;31mSet plaintext error:\033[0m " << e.what() << std::endl;
        throw; // Re-throw the exception for the caller to handle
    }
}
...
void monoalphabetic::set_ciphertext(const std::string &c)
{
    ...
    {
        std::cerr << "\033[1;31mSet ciphertext error:\033[0m " << e.what() << std::endl;
        throw; // Re-throw the exception for the caller to handle
    }
}
